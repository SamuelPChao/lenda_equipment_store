...
export default {
  ...
  computed: {
    totalPages() {
      return Math.ceil(
        this.newsData.length / this.newsPerPage
      )
    }
  },
  methods: {
    async getAllNews() {
      try {
        ...
        if (this.queryString)
          return this.filterNews(this.queryString)
    ...
    dividePage() {
      const startIndex =
        (this.currentPage - 1) * this.newsPerPage
      const endIndex = startIndex + this.newsPerPage
      this.outputNews = this.newsData.slice(
        startIndex,
        endIndex
      )
    },
      ...
      <div
        ...
        :class="{
          ...
          newsBoxPrior:
            currentNews - 1 === i ||
            (currentNews === 0 &&
              i === outputNews.length - 1),
          newsBoxAfter:
            currentNews + 1 === i ||
            (currentNews === outputNews.length - 1 &&
              i === 0)
        }"
